...
function countSyllablesInWord(word: string): number {
  ...
  if (lowerWord.endsWith('le') && !/[aeiouy]/.test(lowerWord.charAt(lowerWord.length - 3))) {
    count++;
  }
...
export function calculateReadability(text: string) {
  ...
  const syllableCount = words.reduce((acc, word) => acc + countSyllablesInWord(word), 0);

  if (wordCount === 0 || sentenceCount === 0) {
    return { fleschKincaid: 0 };
  }

  const fleschKincaid = 206.835 - 1.015 * (wordCount / sentenceCount) - 84.6 * (syllableCount / wordCount);
...
const stopWords = new Set([
  ...
  'will', 'with', 'you', 'your',
  // Ukrainian
  'а', 'але', 'без', 'був', 'була', 'були', 'було', 'в', 'вам', 'вас', 'весь', 'від', 'він',
  'вона', 'вони', 'все', 'всі', 'ви', 'де', 'для', 'до', 'е', 'є', 'ж', 'за', 'з', 'і', 'її', 'їм',
  'й', 'його', 'із', 'ін', 'інших', 'їх', 'його', 'коли', 'крім', 'лише', 'ми', 'на', 'над', 'нас', 'наш', 
  'не', 'нею', 'ні', 'ній', 'них', 'о', 'ось', 'під', 'по', 'при', 'про', 'раз', 'с', 'собі',
  'та', 'там', 'те', 'ти', 'то', 'той', 'тільки', 'тому', 'тут', 'у', 'хто', 'ця', 'це', 'цей', 'ці', 'цю',
  'чи', 'чого', 'що', 'щоб', 'я', 'як'
]);

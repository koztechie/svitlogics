 ...
 */
const TextInput: React.FC<TextInputProps> = ({ text, setText, onAnalyze, isAnalyzing, maxLength }) => {
      ...
      <div className="p-4 border-t border-black bg-white">
        <div className="flex flex-col gap-y-4 sm:flex-row sm:justify-between sm:items-center sm:gap-y-0">
          <div className="flex flex-wrap gap-x-4 gap-y-1 order-2 sm:order-1 text-ui-label font-mono">
            <span className={`text-text-secondary ${isLimitExceeded ? 'text-status-error' : ''}`}>
              {charactersLabel} <strong className="font-medium text-black">{charCount}</strong> / {maxLength}
            </span>
            <span className="text-text-secondary">
              {wordsLabel} <strong className="font-medium text-black">{wordCount}</strong>
            </span>
          </div>
          <button
            type="button" 

...
const ContactSection: React.FC<{ section: typeof content.sections[0] }> = ({ section }) => (
  <section aria-labelledby={section.id} className="border border-black bg-white p-6 rounded-none">
    <h2 id={section.id} className="font-mono font-semibold text-h2-mobile lg:text-h2-desktop text-black mb-4 normal-case">
      {section.title}
    </h2>
    <div className="space-y-4 font-mono font-normal text-body-main leading-body text-black">
      {section.paragraphs?.map((p, index) => <p key={index}>{p}</p>)}
